...
from .utils import get_lyrics, mopidy_api, session
...
class Album(NamedModel):
    ...
    async def get_cover(self):
        try:
            cover_url = await mopidy_api('core.library.get_images', uris=[self.uri])[self.uri][1]['uri']

            fp = BytesIO()
            fp.write(await session.get(cover_url).content)
...
class Track(NamedModel):
    ...
    async def get_lyrics(self):
        self.lyrics = await get_lyrics(self.artists.first(), self.name)
        await database_sync_to_async(self.save)()
    ...
    async def update_from_mopidy(self):
        await get_or_create_from_mopidy(uri=self.uri, force_update=True)
...
async def get_or_create_from_mopidy(track_data=None, uri='', force_update=False):
...
async def add_random(cls):
    if await mopidy_api('core.tracklist.get_length') < 10:
        # TODO await / yield from / async in list comprehension ?
        tracks = await mopidy_api('core.tracklist.get_tracks')
        current = models.Q(uri__in=set(t['uri'] for t in tracks))

        @database_sync_to_async
        def filter_tracks(current):
            ...
            return Track.objects.filter(old_enough, active).exclude(current).order_by('?').first()

        track = await filter_tracks(current)
        if track:
            print(f'Randomly added {track}')
            await mopidy_api('core.tracklist.add', uri=track.uri)
        else:
